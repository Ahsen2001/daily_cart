...
<div class="grid gap-4 md:grid-cols-2">
    ...
    @if(isset($products))
        ...
        <div>
            <x-input-label for="target_id" :value="__('Product')" />
            <select id="target_id" name="target_id" class="mt-1 w-full rounded-md border-gray-300 shadow-sm" required>
                <option value="">{{ __('Select a Product') }}</option>
                @foreach($products as $product)
                    <option value="{{ $product->id }}" @selected(old('target_id', $promotion->target_id) == $product->id)>
                        {{ $product->name }} (LKR {{ number_format($product->price, 2) }})
                    </option>
                @endforeach
            </select>
        </div>
