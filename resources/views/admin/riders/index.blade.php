<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('Rider Approvals') }}</h2>
    </x-slot>
            ...
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div class="space-y-4">
                        @forelse ($riders as $rider)
                            <div class="flex flex-col gap-3 border-b border-gray-100 pb-4 sm:flex-row sm:items-center sm:justify-between">
                                ...
                                <div class="flex gap-2">
                                    <form method="POST" action="{{ route('admin.riders.approve', $rider) }}">
                                        @csrf
                                        @method('PATCH')
                                        <x-primary-button>{{ __('Approve') }}</x-primary-button>
                                    </form>

                                    <form method="POST" action="{{ route('admin.riders.reject', $rider) }}">
                                        @csrf
                                        @method('PATCH')
                        ...
                        @empty
                            <p class="text-sm text-gray-600">{{ __('No riders found.') }}</p>
                        @endforelse
                    </div>
