<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold leading-tight text-gray-800">{{ __('Edit Admin Account') }}</h2>
                ...
                <form method="POST" action="{{ route('super-admin.admins.update', $admin) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <option value="suspended" @selected(old('status', $admin->status) === 'suspended')>{{ __('Suspended') }}</option>
                            <option value="inactive" @selected(old('status', $admin->status) === 'inactive')>{{ __('Inactive') }}</option>
                        </select>
