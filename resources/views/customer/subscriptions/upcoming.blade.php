                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach ($subscriptions as $subscription)
                            <tr><td class="px-4 py-3">{{ $subscription->product?->name }}</td><td>{{ $subscription->vendor?->store_name }}</td><td>{{ $subscription->next_delivery_date?->format('Y-m-d') }}</td><td>{{ $subscription->preferred_delivery_time }}</td><td>{{ $subscription->delivery_address }}</td></tr>
                        @endforeach
                    </tbody>
                </table>
