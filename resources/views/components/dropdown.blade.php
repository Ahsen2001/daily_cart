...
<div class="relative" x-data="{ open: false }" @click.outside="open = false" @keydown.escape.window="open = false" @close.stop="open = false">
    ...
    <div x-show="open"
            ...
            class="absolute z-50 mt-2 {{ $width }} rounded-2xl shadow-lift {{ $alignmentClasses }}"
            style="display: none;"
            @click="open = false">
        <div class="rounded-2xl border border-brand-border ring-1 ring-black/5 {{ $contentClasses }}">
            {{ $content }}
        </div>
    </div>
</div>
